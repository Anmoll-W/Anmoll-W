...
| Title | Covers 
|---|---|
| [Two Models, One Pipeline: Building a Multi-LLM Peer Review Gate That Caught Its Own Silent Bug](https://github.com/Anmoll-W/blog/blob/main/posts/multi-llm-peer-review-gate.md) | A four-mode peer review pipeline that fires on every vault file flipped to status: ready — Claude proposes, Gemini reviews, the gate fails closed if either side disagrees. Two-track auth (API key for headless, OAuth for interactive), free-tier-only on Google AI Pro, output-shape validation against required H2 sections per mode, async dispatcher fan-out under launchd. The verify pass found a silent bug in the system itself — gemini-cli refusing headless runs in untrusted directories with exit 0 and empty stdout — which is the exact bug class wrappers checking exit codes never catch
| [I Cherry-Picked a Viral Cost-Cut Post](https://github.com/Anmoll-W/blog/blob/main/posts/cherry-picking-the-cost-post.md) | A viral post claimed $750 to $100 a month on Claude Code through seven specific levers. Adopted four with documented mechanisms (cache discipline, /effort per prompt, input-format preferences, subagent model routing) and rejected three that turned out to be marketing claims (GLM-5.1 routing, code-graph multipliers, autocompact override). Covers the bug an eval agent caught — Plan to Opus routing backfires on Max $100 because quota tier is not the same as price tier — and the three-question framework for cherry-picking any cost-cut post that lands in your feed
| [The Eval Layer Caught Me Violating My Own Rules](https://github.com/Anmoll-W/blog/blob/main/posts/the-eval-layer-caught-me.md) | Installing Karpathy's four AI coding principles across thirty CLAUDE.md files at three levels — and watching the eval agent catch a naming inconsistency, a duplicate file, and Tier B bloat that violated Simplicity First. Covers the cascade-vs-gradient decision, why discipline scales differently than rules, and why every system that makes decisions needs an eval layer that catches its own drift
| [AI Runners That Remember](https://github.com/Anmoll-W/blog/blob/main/posts/ai-runners-that-remember.md) | Six scheduled runners with persistent cross-run memory — a per-runner Strategy file that writes behavioral instructions for the next run, an SRS queue that surfaces past mistakes at decision time, and a 2am sleeptime consolidator that annotates patterns with live strength signals. Implementable with file I/O and Claude Code CLI, no SDK required
| [Google Analytics Was Fine. Opening It Was Not.](https://github.com/Anmoll-W/blog/blob/main/posts/google-analytics-daily-digest.md) | Same pattern as the support digest, applied to GA4 — a Vercel cron pulls yesterday's analytics, runs it through Claude Haiku prompted as a product manager with explicit 2026 priorities, and emails a six-section digest at 08:00 IST. Covers the funnel baseline mechanism, the Priya-voiced insight block, and three live-only gotchas that tests could not catch
